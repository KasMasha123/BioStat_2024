---
title: "Ошибка вычисления доли"
author: "Kасьянова Мария"
date: "2024-10-08"
...
Сгенерируем "генеральную совокупность", в которой с частотами *p_true* и *1 - p_true* происходят события А (полное выздоровление) или В (неполное выздоровление или смерть).
...
Наше исследование генеральной совокупности выглядело бы следующим образом: берем нескольких подопытных и считаем, какая доля из них выздоровела. (*replace = T* имитирует бесконечную генеральную совокупность, в которой после исключения одного человека, не меняются вероятности событий А и В)
```{r}
...
Но, в прошлый раз нам могло повезти (или не повезти), поэтому повторим исследование много раз и возьмем среднее по всем оценкам:
...
Наконец, мы можем вычислить размер ошибки (отклонения от истинного значения)
```{r}
...
Несложно заметить, что чем больше размер выборки, тем меньше ошибка, причем убывает она нелинейно.

## Зависимость от истинной вероятности

Осталось узнать, как меняется ошибка в зависимости от истинной вероятности события А:
...
На этот раз ошибка больше в том случае, когда оба события А и В происходят часто (если какое-то из этих событий очень редкое, то ошибка уменьшается)